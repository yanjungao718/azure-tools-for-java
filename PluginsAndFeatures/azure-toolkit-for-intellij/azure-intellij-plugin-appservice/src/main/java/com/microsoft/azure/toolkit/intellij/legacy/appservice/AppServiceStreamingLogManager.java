...
public enum AppServiceStreamingLogManager {
    ...
    public void closeStreamingLog(Project project, String appId) {
        final AzureString title = OperationBundle.title("appservice.close_log_stream.app", ResourceUtils.nameFromResourceId(appId));
    ...
    private void showAppServiceStreamingLog(Project project, String resourceId, ILogStreaming logStreaming) {
        ...
        final AzureString title = OperationBundle.title("appservice.open_log_stream.app", ResourceUtils.nameFromResourceId(resourceId));
        AzureTaskManager.getInstance().runInBackground(new AzureTask(project, title, false, () -> {
            try {
